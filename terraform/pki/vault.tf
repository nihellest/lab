...
resource "vault_pki_secret_backend_root_cert" "ca_2025" {
  ...
  type        = "internal"
  common_name = var.cert_config.ca_name
  ttl         = "315360000"
  issuer_name = var.cert_config.ca_issuer_name
}
...
resource "vault_pki_secret_backend_role" "role" {
  ...
  key_type         = "rsa"
  key_bits         = 4096
  allowed_domains  = var.cert_config.allowed_domains
...
resource "vault_pki_secret_backend_root_sign_intermediate" "intermediate" {
  backend     = vault_mount.pki.path
  common_name = var.cert_config.int_name
...
resource "vault_pki_secret_backend_issuer" "intermediate" {
  ...
  issuer_name = var.cert_config.int_issuer_name
}
